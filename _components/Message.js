...
export function Message({ item, index, onLayout }) {
    const listComponent = useMemo(() => {
        var files = item.files !== null ? item.files.split(',') : null;
        return (
            <View style={[styles.message, item.type !== "i" ? styles.messageLeft : styles.messageRight]} key={index} onLayout={onLayout}>
                ...
                <View style={[styles.messageText, item.type !== "i" ? styles.messageTextLeft : styles.messageTextRight]}>
                    ...
                    {files !== null ? files.map((itm, inx) => <File index={inx} files={files} style={item.type !== "i" ? styles.fileLeft : styles.fileRight} mb={files.length  === inx + 1 ? 0 : 8} title={itm} url={`${configApi.apiUrl.slice(0, -4)}/storage/${itm}`} />) : null}
                </View>
            </View>
        );
    }, [item]);
